...
export type PlotConfig = {
  x: {x0: number, min: number, max: number, nTicks: number, nthTick: number, length: number, direction: string, zeroPosition: string}, 
  y: {y0: number, min: number, max: number, nTicks: number, nthTick: number, length: number, direction: string, zeroPosition: string}, 
}
...
export const getRanges = (data: Position[]): PlotConfig => {
  ...
  if(oInX)      {zeroPositionX = "inside"}
  if(oSmallerX) {zeroPositionX = "smaller"}
  if(oBiggerX)  {zeroPositionX = "bigger"}
  
  if(oInY)      {zeroPositionY = "inside"}
  if(oSmallerY) {zeroPositionY = "smaller"}
  if(oBiggerY)  {zeroPositionY = "bigger"}
  ...
  const config = {
    x:{x0:5, min:xMin, max:xMax, nTicks:nTicksX, nthTick:nthTickX, length: 90, direction: directionX, zeroPosition: zeroPositionX},
    y:{y0:20, min:yMin, max:yMax, nTicks:nTicksY, nthTick:nthTickY, length: 70, direction: directionY, zeroPosition: zeroPositionY},
  }
